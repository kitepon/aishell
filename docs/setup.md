...
保存済みのデバイス番号（`st_dev`）を現在値と照合しない。この番号はOSの再起動や再マウントで変わり得るため、
編集の開始・復旧・容量予約・Trash退避を拒否する根拠にしない。旧markerや復旧記録はそのまま読める。
同一ボリュームが必要な操作は現在のファイル同士で確認し、パス・inode・内容SHAによる既存の競合検出は維持する。
